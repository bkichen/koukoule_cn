$(function(){
	...
	if(Page == 25)
	{
      ...
      var check = function(){
       ...
       clearInterval(set);
   //      	var contentHeight = $(".g-cms-detail").height();
		 //    $(".g-cms-detail").height(680);
		 //    $(".g-cms-detail").addClass("on-hover");
		 //    $(".m-show-content p").click(function(){
		 //      var btnText = $(this).text().replace(/\s+/g,"")
		 //      if(btnText=="加载全部内容" || btnText=="点击查看更多"){
		 //        $(".g-cms-detail").animate({height:contentHeight},300);
		 //        $(".g-cms-detail").removeClass("on-hover");
		 //        $(this).text("点击收起内容").append('<b class="u-up"></b>')
		 //      }else{
		 //        $(".g-cms-detail").animate({height:680},300);
	...
	if($(".m-img-hide").length>0){
		$(".m-img-hide").each(function(){
			...
			for(i=0;i<n;i++){
				...
				if(imgSrc=="https://www.qqtn.com/skin/NoPic.jpg"){	
					 $(this).find("li .m-hide").eq(i).hide();
				}
			}			
		})
	}	
	//$("#content").before('<div style=" margin:5px 0 0 0;font-size: 16px;    text-align:center;  padding: 0px;"><span style=" font-size:.8em; color:#606160;">长按复制腾牛个性微信公众号 →</span><span style="font-size:.9em; color:#19b5fe;">qqtn-gexing</span></div>');
	//addla()
})
...
//	}
	...
	if(/iphone|ipad/i.test(navigator.userAgent)){//苹果底飘
		//document.writeln('<script src=\"https:\/\/s.ddhtek.com\/cc-4801-4401.js\"><\/script>');
	//document.write('<script src="http://mob.coolnay.com/show.php?mmm=mmm&zoneid=1258"></script>');
	}else{
		//文章页底部
   ...
   downLoad :function(){
	   var isAds=false; //是否为商务包
		...
		if(browser.versions.ios && !catearrIos.in_array(_pageinfo.categroyId) ) { //如果是苹果设备 不是苹果分类
				...
				if(!checkURL(pic)){pic="https://www.qqtn.com/"+pic};
...
function img(){
	...
	if(touimgnum>1){
		$("#content img").each(function(i){
			 ...
			 var imguleft = imgurl.split(".")[2].split("_")[0].substr(11,32)
...
function gonext(){
	...
	$("#content").append(gonextxt);
	var gonexturl = $("#topnav li a:first").attr("href")
	$(".m-nexta").attr("href",gonexturl)
	if(_pageinfo.categroyId == 9){
		$(".m-nexta").attr("href","/bq.html")
	}else if(_pageinfo.categroyId == 18){
		$(".m-nexta").attr("href","/ss.html")
	}else if(_pageinfo.categroyId == 20){
		
	}else if(_pageinfo.categroyId == 23){
		$(".m-nexta").attr("href","/qm.html")
	}else if(_pageinfo.categroyId == 28){
		$(".m-nexta").attr("href","/wm.html")
	}else if(_pageinfo.categroyId == 72){
		$(".m-nexta").attr("href","/tp.html")
	}else if(_pageinfo.categroyId == 74){
		$(".m-nexta").attr("href","/pf.html")
	}
}
